import { solutionCollection, db, riskCollection, voteCollection } from "@/models/name";
    ...
    try {
        ...
        if (response.documents.length > 0) {
            ...
            const questionOrAnswer = await databases.getDocument(
                db,
                type === "question" ? riskCollection : solutionCollection,
                typeId
            );
        ...
        if (response.documents[0]?.voteStatus !== voteStatus) {
            ...
            const questionOrAnswer = await databases.getDocument(
                db,
                type === "risk" ? riskCollection : solutionCollection,
                typeId
            );
